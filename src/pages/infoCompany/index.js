...
function InfoCompanyManage(){
  ...
  return (
    <>
      ...
      <div>{idCompany ? <div>{isDisabled ? <Button onClick={()=> {setIsDisabled(false)}} icon={<EditOutlined />}>Chỉnh sửa</Button> : <Button onClick={()=> {setIsDisabled(true)}} icon={<CloseOutlined />}>Hủy</Button>}</div> : <Button icon={<PlusOutlined />} onClick={showModal}>Thêm công ty</Button> }</div>
      <Modal
